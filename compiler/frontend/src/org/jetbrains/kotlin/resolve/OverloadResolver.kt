...
class OverloadResolver(
    ...
    private fun getPossibleRedeclarationGroups(
    ...
    private fun findRedeclarations(members: Collection<DeclarationDescriptorNonRoot>): Set<DeclarationDescriptorNonRoot> {
    ...
    private fun reportRedeclarations(redeclarations: Set<DeclarationDescriptorNonRoot>) {
        if (redeclarations.isEmpty()) return

        for (memberDescriptor in redeclarations) {
            val ktDeclaration = DescriptorToSourceUtils.descriptorToDeclaration(memberDescriptor) ?: continue
            ...
            when (memberDescriptor) {
                ...
                is FunctionDescriptor -> {
                    trace.report(Errors.CONFLICTING_OVERLOADS.on(ktDeclaration, memberDescriptor, memberDescriptor.containingDeclaration))
                }
            }
        }
    }
